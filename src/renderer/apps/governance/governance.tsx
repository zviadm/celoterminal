...
import {
	...
	TXFinishFunc,
	TXFunc,
} from "../../components/app-definition";
	...
	const { isFetching, fetched, refetch } = useOnChainState(
		React.useCallback(
			async (kit: ContractKit) => {
				...
				const proposals = Promise.all([dequeue, durations]).then(
					([dequeue, durations]) => {
						return concurrentMap(4, dequeue, async (p) => {
							...
							const timeUntilExecution = secondsToDurationString(
	...
	const handleUpvote = (proposalID: BigNumber) => {
		props.runTXs(
			async (kit: ContractKit) => {
				...
				if (
					fetched.upvotes.find((u) =>
						u.proposalID.eq(fetched.upvoteRecord.proposalID)
					)
				) {
		...
		<AppContainer>
			...
			{fetched && (
				<>
					<AppSection>
						<Alert severity="info">
							Use <Link href="https://celo.stake.id">celo.stake.id</Link> to
							view more in-depth information about all past and currently active
					...
					{fetched.proposals.length === 0 && fetched.upvotes.length === 0 && (
						<AppSection>
							<Alert severity="info">
								There are no active governance proposals to upvote or vote right
								now.
							</Alert>
						</AppSection>
					)}
					{fetched.proposals.length > 0 && (
						<AppSection>
							{canVote ? (
								<Alert severity="info">
									Voting on a proposal disables unlocking of CELO until proposal
									is finished.
								</Alert>
							) : (
								<Alert severity="error">
									To participate in governance, you need to first register your
									address and lock CELO using the Locker app.
								</Alert>
							)}
							<Table size="small">
								<TableHead>
								...
								<TableBody>
									{fetched.proposals.map((p) => {
										const vote = fetched.voteRecords.find((v) =>
											v.proposalID.eq(p.proposalID)
										);
										if (!p.votes) {
											return <></>;
										}
										const total = p.votes.Yes.plus(p.votes.No).plus(
											p.votes.Abstain
										);
										return (
													...
													<Table size="small">
														<TableBody>
															<TableRow>
																...
																<TableCell width="100%">
																	{fmtAmount(p.votes.Yes, "CELO", 0)}
																</TableCell>
																<TableCell>
																	{p.votes.Yes.div(total)
																		.multipliedBy(100)
																		.toFixed(0)}
																	%
																</TableCell>
															...
															<TableRow>
																...
																<TableCell>
																	{fmtAmount(p.votes.No, "CELO", 0)}
																</TableCell>
																<TableCell>
																	{p.votes.No.div(total)
																		.multipliedBy(100)
																		.toFixed(0)}
																	%
																</TableCell>
															...
															<TableRow>
																...
																<TableCell>
																	{fmtAmount(p.votes.Abstain, "CELO", 0)}
																</TableCell>
																<TableCell>
																	{p.votes.Abstain.div(total)
																		.multipliedBy(100)
																		.toFixed(0)}
																	%
																</TableCell>
														...
														{(
															...
															<Box key={v} marginBottom={0.5}>
																<Button
																	...
																	style={{ width: 100 }}
																	variant={
																		vote?.value === v ? "contained" : "outlined"
																	}
																	color={
																		vote?.value === v ? "primary" : "default"
																	}
																	disabled={!canVote}
																	onClick={() => {
																		handleVote(p.proposalID, v);
																	}}
																>
																	{v}
																</Button>
							...
							<Table size="small">
								...
								<TableBody>
									{fetched.upvotes.map((v) => {
										const isUpvoted = v.proposalID.eq(
											fetched.upvoteRecord.proposalID
										);
										return (
...
const ProposalID = (props: { id: BigNumber }) => {
	return (
		<Link href={`https://celo.stake.id/#/proposal/${props.id.toString()}`}>
			{props.id.toString()}
		</Link>
	);
};
