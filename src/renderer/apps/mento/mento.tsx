...
const MentoApp = (props: {
	...
	const [stableToken, setStableToken] = useLocalStorageState<StableToken>(
		"terminal/mento/stable-token",
		StableToken.cUSD
	);
	const [side, setSide] = useLocalStorageState<"buy" | "sell">(
		"terminal/mento/side",
		"sell"
	);
	...
	const [slippagePct, setSlippagePct] = useLocalStorageState(
		"terminal/mento/max-slippage",
		slippageOptions[1]
	);
	...
	const { isFetching, fetched, refetch } = useExchangeOnChainState(
		account,
		stableToken
	);
	...
	const [anchorToken, setAnchorToken] = React.useState<"celo" | "stable">(
		"celo"
	);
	React.useEffect(() => {
		if (!fetched) {
		...
		if (anchorToken === "celo") {
			const stableAmountN = calcStableAmount(
				...
				fetched.celoBucket,
				fetched.stableBucket,
				fetched.spread
			);
			setStableAmount(
				fmtTradeAmount(
					stableAmountN,
					side === "sell" ? BigNumber.ROUND_DOWN : BigNumber.ROUND_UP
				)
			);
		} else {
	...
	const canTrade =
		...
		stableAmountN.gt(0) &&
		((side === "sell" &&
			fetched.celoBalance.shiftedBy(-coreErc20Decimals).gte(celoAmount)) ||
			(side === "buy" &&
				fetched.stableBalance.shiftedBy(-coreErc20Decimals).gte(stableAmount)));
	...
	return (
		<AppContainer>
			...
			<AppSection>
				<Box display="flex" flexDirection="row" alignItems="flex-end">
					...
					<Select
						value={stableToken}
						onChange={(event) => {
							handleChangeStable(event.target.value as StableToken);
						}}
					>
				...
				<Box
					display="flex"
					flexDirection={side === "sell" ? "column" : "column-reverse"}
				>
					<NumberInput
					...
					<NumberInput
						...
						label={
							!fetched || side !== "buy"
								? `${stableToken}`
								: `${stableToken} (max: ${fmtAmount(
										fetched.stableBalance,
										coreErc20Decimals
								  )})`
						}
						InputLabelProps={{
				...
				>
					<Typography color="textSecondary" variant="caption">
						{price.isNaN() ? (
							...
							`Price: ${price.toFixed(4)} ${stableToken}`
						)}
					</Typography>
				</Box>
				<Box
			...
			<AppSection>
				<TradeHistory
					stableToken={stableToken}
					events={exchangeHistory.fetched}
				/>
			</AppSection>
		</AppContainer>
	);
